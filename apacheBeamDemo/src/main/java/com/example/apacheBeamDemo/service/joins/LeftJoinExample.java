...
public class LeftJoinExample implements Serializable {
    public void usingLeftJoin(){
        ...
        PCollection<String> output = result.apply(ParDo.of( new DoFn<KV<String,CoGbkResult>,String>(){
                                                                ...
                                                                public void processElement(ProcessContext c){
                                                                    ...
                                                                    for(String order : userOrderTable){
                                                                        if(userNameTable.iterator().hasNext()){
                                                                            for(String user :userNameTable){
                                                                                c.output(key+","+user+","+order);
                                                                            }
                                                                        }
                                                                        else{
                                                                            c.output(key+","+null+","+order);
                                                                        }
        ...
        output.apply(TextIO.write().to("/Users/restynasimbwa/apacheBeam/output_data/left_join_output.csv")
                .withNumShards(1)
                .withSuffix(".csv"));
